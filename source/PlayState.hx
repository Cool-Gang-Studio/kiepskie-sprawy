...
class PlayState extends MusicBeatState
{
	...
	var weekOneStartScreen:FlxSprite;
	...
	override public function create()
	{
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				/* // mah, i'll make a hotfix ig - teotm */
				case 'full':
					startVideoThenShowSongName('cut1', 'now-playing-assets/full');
				case 'karwasz-twarz':
					startVideoThenShowSongName('cut2', 'now-playing-assets/karwasz-twarz');
				case 'ogor':
					startVideoThenShowSongName('cut3', 'now-playing-assets/ogor');
	...
	public function startVideoThenShowSongName(name:String, image:String)
	{
		#if VIDEOS_ALLOWED
		inCutscene = true;

		var videoFilePath:String = Paths.video(name);
		#if sys
		if(!FileSystem.exists(videoFilePath))
		#else
		if(!OpenFlAssets.exists(videoFilePath))
		#end
		{
			FlxG.log.warn('Couldnt find video file: ' + videoFilePath);
			startAndEnd();
			return;
		}
		...
		var video:VideoHandler = new VideoHandler();
		#end
		video.playVideo(videoFilePath);
		video.finishCallback = function()
		{
			var startScreen:FlxSprite = new FlxSprite(150, -100, Paths.image(image, 'shared'));
			startScreen.cameras = [camHUD];
			startScreen.scale.set(1.4, 1.4);
			add(startScreen);
			FlxTween.tween(startScreen, {alpha: 0}, 1.4, {
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				switch (songName)
				{
					case 'full' | 'karwasz-twarz' | 'ogor':
						{
							if (weekOneStartScreen != null) {
								FlxTween.tween(weekOneStartScreen, {alpha: 0}, 0.5, {
									ease: FlxEase.circInOut,
									onComplete: function(twn:FlxTween) {
										remove(weekOneStartScreen);
								}});
							}
						}
				}
